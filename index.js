...
// Targeted solution to remove only the Synapse header tab
function removeSynapseTab() {
  function removeTab() {
    // Find header tabs, not toolbar items
    document.querySelectorAll('.cp__header-tab, [data-tab-id], .tab').forEach(tab => {
      // Check if this contains "Synapse" text but is NOT in a toolbar/settings area
      if (tab.textContent.includes('Synapse') && 
          !tab.closest('.toolbar') && 
          !tab.closest('[aria-label="Settings"]') && 
          !tab.closest('.ui__dropdown-menu')) {
          
        // Find and click close button
        const closeBtn = tab.querySelector('button, .close');
        if (closeBtn) {
          closeBtn.click();
          console.log("Removed Synapse tab");
        }
      }
    });
  }
  ...
  [2000, 5000, 10000].forEach(delay => setTimeout(removeTab, delay));
}
// Comprehensive solution to completely prevent Synapse from appearing in header
function removeSynapseFromHeader() {
  console.log("Setting up Synapse header removal");
  
  // METHOD 1: Prevent initial tab creation by overriding APIs
  if (logseq.App && logseq.App.pushState) {
    const originalPushState = logseq.App.pushState;
    logseq.App.pushState = function(type, opts) {
      // Check if this is a Synapse tab being created
      if (opts && (
          opts.page === 'Synapse' || 
          opts.name === 'Synapse' || 
          (typeof opts.page === 'string' && opts.page.includes('Synapse')) ||
          (typeof opts.name === 'string' && opts.name.includes('Synapse'))
      )) {
        console.log("Prevented Synapse tab creation via pushState");
        return Promise.resolve(); // Return empty promise
      }
      // Otherwise call original method
      return originalPushState.call(this, type, opts);
    };
  }
  
  // METHOD 2: Remove any existing tabs
  function removeSynapseTabs() {
    const tabElements = document.querySelectorAll('.cp__header-tab, .tab, [data-tab-id]');
    
    tabElements.forEach(tab => {
      if (tab.textContent.includes('Synapse')) {
        // Find close button and click it
        const closeBtn = tab.querySelector('button, .close-btn, [aria-label="Close"]');
        if (closeBtn) {
          closeBtn.click();
          console.log("Removed existing Synapse tab");
        }
      }
    });
  }
  
  // Run removal at startup and periodically
  setTimeout(removeSynapseTabs, 1000);
  setTimeout(removeSynapseTabs, 3000);
  setTimeout(removeSynapseTabs, 6000);
  setInterval(removeSynapseTabs, 30000);
}


// Solution to remove Synapse from header
function hideSynapseTab() {
  // Function to find and remove Synapse tabs
  function removeTab() {
    document.querySelectorAll('.cp__header-tab, .tab, [data-tab-id]').forEach(tab => {
      if (tab.textContent.includes('Synapse')) {
        const closeBtn = tab.querySelector('button, .close, [aria-label="Close"]');
        if (closeBtn) {
          closeBtn.click();
          console.log("Removed Synapse tab");
        }
  ...
  // Set up multiple removal attempts
  [1000, 3000, 6000, 10000].forEach(delay => 
    setTimeout(removeTab, delay)
  );
  
  // Also set up a periodic check every 30 seconds
  setInterval(removeTab, 30000);
}

// Simple solution to unpin Synapse tab on startup
function unpinSynapseOnStartup() {
  // Try to unpin after LogSeq has fully loaded (multiple attempts)
  [3000, 6000, 9000].forEach(delay => {
    setTimeout(() => {
      // Find tab elements
      document.querySelectorAll('[data-tab-id], .tab, .cp__header-tab').forEach(tab => {
        if (tab.textContent.includes('Synapse')) {
          // Find close button
          const closeBtn = tab.querySelector('button, .close-btn, [aria-label="Close"]');
          if (closeBtn) {
            closeBtn.click();
            console.log("Clicked close button for Synapse tab");
          }
        }
      });
    }, delay);
...
// Comprehensive solution to unpin Synapse tab on startup
function unpinSynapseTab() {
  console.log("Setting up Synapse tab unpinning");
  
  // Add a setting to control this behavior
  if (logseq.useSettingsSchema) {
    const currentSchema = logseq.settings?.settingsSchema || [];
    
    // Check if our setting already exists
    const settingExists = currentSchema.some(item => item.key === "unpinSynapseTab");
    ...
    if (!settingExists) {
      logseq.useSettingsSchema([
        ...currentSchema,
        {
          key: "unpinSynapseTab",
          type: "boolean",
          title: "Unpin Synapse Tab on Startup",
          description: "Automatically unpin the Synapse tab when LogSeq starts",
          default: true
        }
      ]);
    }
  ...
  // Check if user wants to unpin
  const shouldUnpin = logseq.settings?.unpinSynapseTab !== false; // Default to true
  
  if (shouldUnpin) {
    // Set up multiple attempts with increasing delays
    [2000, 5000, 10000].forEach(delay => {
      setTimeout(attemptUnpinning, delay);
    });
  }
  
  // Function to try multiple unpinning approaches
  function attemptUnpinning() {
    console.log("Attempting to unpin Synapse tab...");
    
    // Try to find and click unpin buttons in the UI
    const unpinButtons = document.querySelectorAll('button.close-tab, button.unpin-tab');
    
    let found = false;
    unpinButtons.forEach(button => {
      const parentTab = button.closest('.tab') || button.parentElement.closest('.tab');
      
      if (parentTab && parentTab.textContent.includes('Synapse')) {
        console.log("Found Synapse tab with unpin button, clicking...");
        button.click();
        found = true;
      }
    });
    
    // If we couldn't find a button, try a more general approach
    if (!found) {
      // Find all tabs
      const allTabs = document.querySelectorAll('.cp__header-tab, .tab, [data-tab-id]');
      
      allTabs.forEach(tab => {
        if (tab.textContent.includes('Synapse')) {
          console.log("Found Synapse tab:", tab);
          
          // Look for any button or close element within this tab
          const closeEl = tab.querySelector('button, .close, [aria-label="Close"], [aria-label="Unpin"]');
          
          if (closeEl) {
            console.log("Clicking close/unpin element:", closeEl);
            closeEl.click();
            found = true;
          }
        }
    ...
    if (!found) {
      console.log("Could not find Synapse tab to unpin in this attempt");
    }
  }
}

// Last resort solution - force style all Synapse header candidates
function emergencyTargetAllSynapseHeader() {
  console.log("EMERGENCY: Applying style to all potential header elements...");
  
  // Be very aggressive and style any element with text "Synapse" near the top
  function forceStyleAllCandidates() {
    // Candidate 1: Style any element with exact text "Synapse" at the top
    document.querySelectorAll('*').forEach(el => {
      if (el.textContent.trim() === 'Synapse') {
        const rect = el.getBoundingClientRect();
        if (rect.top < 60) {
          el.style.fontSize = '0.7em';
          console.log("Styled exact text Synapse element:", el);
        }
      }
    });
    
    // Candidate 2: Style specific classes commonly used for headers
    ...
    headerSelectors.forEach(selector => {
      document.querySelectorAll(selector).forEach(el => {
        ...
        const rect = el.getBoundingClientRect();
        if (rect.top < 60) {
          // Apply style if it contains Synapse
          if (el.textContent.includes('Synapse')) {
            el.style.fontSize = '0.7em';
            console.log(`Styled ${selector} element:`, el);
          }
        }
      });
    });
  ...
  setTimeout(forceStyleAllCandidates, 1000);
  setTimeout(forceStyleAllCandidates, 3000);
}


// Add this debugging function to your code
function debugHeaderElements() {
  console.log("Starting header element debugging...");
  
  // 1. Find all 'Synapse' text nodes with their details
  console.log("STEP 1: Finding all 'Synapse' text nodes");
  ...
  // Create an array to store all found nodes
  const foundNodes = [];
  
  while(textNode = walker.nextNode()) {
    if (textNode.nodeValue.trim() === "Synapse") {
      count++;
      const parent = textNode.parentElement;
      
      // Get useful info about this element
      const info = {
        node: textNode,
        parent: parent,
        position: parent ? {
      ...
      // Get class names of all ancestors
      if (parent) {
        let current = parent.parentElement;
        while (current) {
          if (current.className) {
            info.ancestorClasses.push({
      ...
      foundNodes.push(info);
      
      console.log(`Found Synapse text node ${count}:`, info);
    }
  }
  
  console.log(`Total Synapse text nodes found: ${count}`);
  
  // 2. Add temporary visual indicators to help identify elements
  console.log("STEP 2: Adding visual indicators to elements");
  
  foundNodes.forEach((info, index) => {
    if (info.parent) {
      // Save original styles
      const originalStyles = {
        outline: info.parent.style.outline,
        background: info.parent.style.backgroundColor,
        position: info.parent.style.position,
        zIndex: info.parent.style.zIndex
      };
      
      // Add visible indicator
      info.parent.style.outline = '3px solid red';
      info.parent.style.backgroundColor = 'yellow';
      info.parent.style.position = 'relative';
      info.parent.style.zIndex = '9999';
      
      // Add label
      const label = document.createElement('div');
      label.textContent = `Synapse ${index + 1}`;
      ...
      info.parent.appendChild(label);
      
      console.log(`Added visual indicator to element ${index + 1}`);
      
      // Restore original styles after 10 seconds
      setTimeout(() => {
        info.parent.style.outline = originalStyles.outline;
        info.parent.style.backgroundColor = originalStyles.background;
        info.parent.style.position = originalStyles.position;
        info.parent.style.zIndex = originalStyles.zIndex;
        info.parent.removeChild(label);
      }, 10000);
    }
  });
  
  console.log("Visual indicators will disappear after 10 seconds");
  
  // Return information about all found nodes
  return foundNodes;
}

// Add a button to manually trigger the debug process
function addDebugButton() {
  const button = document.createElement('button');
  button.textContent = 'Debug Synapse Header';
  ...
  console.log("Debug button added to bottom-right corner");
}

// DOM-only approach to target just the main header Synapse text
function makeHeaderSynapseSmaller() {
  // Function to find and modify the exact Synapse text in main header
  const modifyMainHeaderSynapseText = () => {
    console.log("Searching for main header Synapse text...");
    
    // Find all text nodes with content "Synapse"
    const walker = document.createTreeWalker(
      document.body,
      NodeFilter.SHOW_TEXT,
      null,
    ...
    while(textNode = walker.nextNode()) {
      if (textNode.nodeValue.trim() === "Synapse") {
        ...
        if (parent) {
          // Check element position (top of screen)
          const rect = parent.getBoundingClientRect();
          
          // Check if element is in the main header (at top of page)
          if (rect.top < 50) {
            // Check it's not in sidebar by checking for sidebar classes up the tree
            let inSidebar = false;
            let current = parent;
            ...
            while (current && current !== document.body) {
              if (current.className && 
                  ...
                  (current.className.includes('sidebar') || 
                   current.className.includes('right-side'))) {
                inSidebar = true;
                break;
              }
              current = current.parentElement;
            ...
            // If not in sidebar, apply styling
            if (!inSidebar) {
              parent.style.fontSize = "0.2em";
              parent.style.fontWeight = "normal";
              modifiedAny = true;
              console.log("Modified main header Synapse:", parent);
            }
          }
        }
      }
  ...
  // Run the function a few times to catch any delays in DOM loading
  setTimeout(() => {
    const modified = modifyMainHeaderSynapseText();
    if (!modified) {
      console.log("First attempt didn't find the element, trying again...");
      setTimeout(modifyMainHeaderSynapseText, 2000);
    }
  }, 1000);
}

// Solution that only targets Synapse text in the header
function reduceSynapseHeaderSize() {
  // Apply CSS targeting likely Synapse header selectors
  logseq.provideStyle(`
    /* Target possible Synapse header text elements */
    div.cp__header span:not([class*="date"]),
    div.cp__header > div:not([class*="title"]):not([class*="page"]),
    div.cp__header div[title="Synapse"] {
  ...
  setTimeout(() => {
    // Find text nodes with exactly "Synapse" content in the header area
    const walker = document.createTreeWalker(
      document.body,
      NodeFilter.SHOW_TEXT,
      null,
    ...
    while(textNode = walker.nextNode()) {
      if (textNode.nodeValue.trim() === "Synapse") {
        const parent = textNode.parentElement;
        if (parent) {
          // Check if it's in the header area (top of the page)
          const rect = parent.getBoundingClientRect();
          if (rect.top < 60) {
            parent.style.fontSize = "0.8em";
            console.log("Modified Synapse header text size");
          }
        }
      }
    }
...
// Combined approach with multiple techniques
function improveSynapseHeader() {
  console.log("Applying multiple header improvement techniques");
  
  // 1. Apply CSS to reduce header size
  logseq.provideStyle(`
    /* Target likely header elements */
    .cp__header-title, 
    .title,
    .page-title,
    div.cp__header div,
  ...
  // 2. Also set up direct DOM manipulation as backup
  const modifyHeader = () => {
    // Look for elements containing exactly "Synapse" text
    const walker = document.createTreeWalker(
      document.body,
      NodeFilter.SHOW_TEXT,
      null,
    ...
    while(textNode = walker.nextNode()) {
      if (textNode.nodeValue.trim() === "Synapse") {
        const parent = textNode.parentElement;
        if (parent && !parent.hasAttribute('data-size-modified')) {
          parent.style.fontSize = "0.7em";
          parent.setAttribute('data-size-modified', 'true');
          console.log("Modified Synapse header text size in: ", parent);
        }
      }
    }
  };
  ...
  const intervalId = setInterval(modifyHeader, 3000);
  
  // Stop checking after 1 minute to avoid performance issues
  setTimeout(() => clearInterval(intervalId), 60000);
}

// Simple solution to reduce Synapse header size
function adjustHeaderSize() {
  // Add CSS to reduce size of header elements
  logseq.provideStyle(`
  ...
  // As a backup, set up a one-time direct sizing
  setTimeout(() => {
    document.querySelectorAll('div, span, h1, h2').forEach(el => {
      if (el.textContent === "Synapse" && el.children.length === 0) {
        el.style.fontSize = "0.7em";
      }
    });
  }, 2000);
...
function testAndDebugStyles() {
  console.log("=============================================");
  console.log("Starting Synapse header style debugging...");
  console.log("=============================================");
  
  // Test with very visible styles to verify CSS injection works
  logseq.provideStyle(`
    /* Test 1: This should make block content have red borders - very noticeable */
    .block-content {
      border-left: 2px solid red !important;
    }
    
    /* Test 2: Very aggressive targeting for "Synapse" text with extreme styling */
    div:not([data-synapse-styled]),
    span:not([data-synapse-styled]),
    h1:not([data-synapse-styled]),
    h2:not([data-synapse-styled]),
    a:not([data-synapse-styled]) {
      color: red !important;
      font-size: 0.7em !important;
...
function addCustomHeaderStyles() {
  // Add custom styles to target various header elements
  logseq.provideStyle(`
    /* Target potentially multiple header elements to ensure we catch the right one */
    .cp__header-tab-title,
    .cp__right-sidebar-topbar h1.title,
    .cp__header-title,
    .cp__header .page-title,
    .cp__header-plugin-name,
    div[data-ref='page-title'] {
      font-size: 0.7em !important;
    }
    
    /* More aggressive approach - target all headers at the top */
    .cp__header h1, 
    .cp__header h2, 
    .cp__header-title > div, 
    .cp__header-title span {
      font-size: 0.7em !important;
    }
  `);
}
